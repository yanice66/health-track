...
const isTextEditable = (element): boolean => {
...
const extractDirectTextContent = (element): string => {
  let text = "";
  ...
  useEffect(() => {
    if (isVisualEditMode) {
      ...
      window.parent.postMessage(
        { type, msg: "VISUAL_EDIT_MODE_ACK", active: true },
        "*"
      );

      // Also send a special message to indicate this was restored from localStorage
      window.parent.postMessage(
        { type, msg: "VISUAL_EDIT_MODE_RESTORED", active: true },
        "*"
      );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      if (parent) {
        ...
        newWidth = Math.max(
          20,
          exceedsWidth ? newWidth .min(newWidth, maxWidth)
        );

        newHeight = Math.max(
          20,
          exceedsHeight ? newHeight .min(newHeight, maxHeight)
        );
      } else {
  ...
  useEffect(() => {
    ...
    function onMsg(e) {
      ...
      if (e.data.msg === "VISUAL_EDIT_MODE" && "active" in e.data) {
        ...
        window.parent.postMessage(
          { type, msg: "VISUAL_EDIT_MODE_ACK", active: newMode },
          "*"
        );
